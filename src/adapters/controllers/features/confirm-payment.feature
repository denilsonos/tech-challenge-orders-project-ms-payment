Feature: Confirm Payment
  ...
  Scenario: Successfully confirm payment for an order
    Given an existing payment with id 123 and an existing order with id 456
    When the confirm method is called with { "paymentId": 123, "orderId": 456 }
    Then the payment for order 456 is confirmed and the order status is set to Received

  Scenario: Attempt to confirm payment with invalid payment payload
  ...
  Scenario: Attempt to confirm payment for a non-existent payment
    Given a non-existent payment with id 789 and an existing order with id 456
    When the confirm method is called with { "paymentId": 789, "orderId": 456 }
    Then a BadRequestException with message "Payment identifier 789 is invalid!" is thrown

  Scenario: Attempt to confirm payment for a payment that has already been made
  ...
  Scenario: Attempt to confirm payment for a non-existent order
    Given an existing payment with id 123 and a non-existent order with id 987
    When the confirm method is called with { "paymentId": 123, "orderId": 987 }
    Then a BadRequestException with message "Order identifier 987 is invalid!" is thrown

  Scenario: Attempt to confirm payment for an order that does not have a pending payment
